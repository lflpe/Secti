...
export const EditarCategorias = () => {
  ...
  const carregarCategoria = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      setErro(null);
      const tag = await tagService.obterPorId(parseInt(id));
      setNome(tag.nome);
      setAtivo(tag.ativo);
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await tagService.editar(parseInt(id!), {
        nome: nome.trim(),
        ativo: ativo,
      });
      ...
      <PrivateLayout>
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="bg-white shadow-sm rounded-lg border border-gray-200 p-12 text-center">
            <p className="text-sm text-gray-500">Carregando categoria...</p>
          </div>
        </div>
      </PrivateLayout>
    );
    ...
    <PrivateLayout>
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Editar Categoria</h1>
            <p className="text-gray-600 mt-2">Atualize os dados da categoria</p>
          </div>
          <button
            type="button"
        ...
        {erro && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-sm font-medium text-red-800">Erro ao atualizar categoria</h3>
          ...
          <div>
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                disabled={isSubmitting}
                className="w-4 h-4 text-[#0C2856] rounded focus:ring-2 focus:ring-[#0C2856] disabled:cursor-not-allowed"
              />
              <span className="text-sm font-medium text-gray-700">Ativa</span>
            </label>
            <p className="text-xs text-gray-500 mt-2 ml-7">Desmarque para inativar a categoria</p>
          </div>
