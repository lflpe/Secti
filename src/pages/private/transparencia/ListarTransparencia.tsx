...
const transparenciaMock: Documento[] = [
  {
    id: 1,
    nome: 'Relatório de Gestão Fiscal 2024',
    tipo: 'pdf',
    tamanho: '3.2 MB',
  ...
  {
    id: 2,
    nome: 'Demonstrativo de Despesas Públicas',
    tipo: 'xlsx',
    tamanho: '1.8 MB',
  ...
  {
    id: 3,
    nome: 'Relatório de Atividades 2024',
    tipo: 'pdf',
    tamanho: '4.5 MB',
  ...
  {
    id: 4,
    nome: 'Plano de Metas Institucionais',
    tipo: 'docx',
    tamanho: '2.1 MB',
...
export const ListarTransparencia = () => {
  const [transparencia, setTransparencia] = useState<Documento[]>(transparenciaMock);
  const [filtroTipo, setFiltroTipo] = useState<string>('Todos');
  const [busca, setBusca] = useState<string>('');

  // Filtrar transparência
  const transparenciaFiltrada = transparencia.filter((documento) => {
  ...
  // Função para excluir documento de transparência
  const handleDelete = (id: number) => {
    setTransparencia(transparencia.filter(t => t.id !== id));
  };
    ...
    <PrivateLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              {transparenciaFiltrada.length} {transparenciaFiltrada.length === 1 ? 'documento encontrado' : 'documentos encontrados'}
            </p>
          </div>
          <Link
        ...
        <ListarDocumentosComponent
          documentos={transparenciaFiltrada}
          onDelete={handleDelete}
          emptyStateTitle="Nenhum documento de transparência encontrado"
